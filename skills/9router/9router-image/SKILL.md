...
```bash
curl "$NINEROUTER_URL/v1/models/image" -H "Authorization: Bearer $NINEROUTER_KEY" | jq '.data[].id'
# Per-model params/options (size enum, quality enum, capabilities like edit)
curl "$NINEROUTER_URL/v1/models/info?id=openai/dall-e-3" -H "Authorization: Bearer $NINEROUTER_KEY"
```
