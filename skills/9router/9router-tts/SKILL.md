...
```bash
# 1) List models
curl "$NINEROUTER_URL/v1/models/tts" -H "Authorization: Bearer $NINEROUTER_KEY" | jq '.data[].id'
# 2) Per-model metadata (params, voicesUrl if voice-by-id)
curl "$NINEROUTER_URL/v1/models/info?id=el/eleven_multilingual_v2" -H "Authorization: Bearer $NINEROUTER_KEY"
# 3) List voices (elevenlabs, edge-tts, deepgram, inworld, local-device). Optional ?lang=vi
curl "$NINEROUTER_URL/v1/audio/voices?provider=edge-tts&lang=vi" -H "Authorization: Bearer $NINEROUTER_KEY" | jq '.data[].model'
```
