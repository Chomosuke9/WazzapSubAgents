...
```bash
curl "$NINEROUTER_URL/v1/models/embedding" -H "Authorization: Bearer $NINEROUTER_KEY" | jq '.data[].id'
# Per-model dimensions
curl "$NINEROUTER_URL/v1/models/info?id=openai/text-embedding-3-small" -H "Authorization: Bearer $NINEROUTER_KEY"
```
