...
```bash
curl "$NINEROUTER_URL/v1/models" -H "Authorization: Bearer $NINEROUTER_KEY" | jq '.data[].id'
# Per-model metadata (contextWindow, params)
curl "$NINEROUTER_URL/v1/models/info?id=openai/gpt-4o" -H "Authorization: Bearer $NINEROUTER_KEY"
```
...
JavaScript (built-in Node 24 `fetch`; no package install required):

```js
const response = await fetch(`${process.env.NINEROUTER_URL}/v1/chat/completions`, {
  method: "POST",
  headers: {
...
if (!response.ok) throw new Error(`9Router ${response.status}: ${await response.text()}`);
