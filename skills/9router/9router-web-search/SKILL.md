...
```bash
curl "$NINEROUTER_URL/v1/models/web" -H "Authorization: Bearer $NINEROUTER_KEY" | jq '.data[] | select(.kind=="webSearch") | .id'
# Per-provider params (searchTypes, maxResults, required options like cx for google-pse)
curl "$NINEROUTER_URL/v1/models/info?id=tavily/search" -H "Authorization: Bearer $NINEROUTER_KEY"
```
