...
```bash
curl "$NINEROUTER_URL/v1/models/stt" -H "Authorization: Bearer $NINEROUTER_KEY" | jq '.data[].id'
# Per-model params (language, response_format, prompt, temperature support)
curl "$NINEROUTER_URL/v1/models/info?id=openai/whisper-1" -H "Authorization: Bearer $NINEROUTER_KEY"
```
