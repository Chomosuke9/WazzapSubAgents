...
```bash
curl "$NINEROUTER_URL/v1/models/web" -H "Authorization: Bearer $NINEROUTER_KEY" | jq '.data[] | select(.kind=="webFetch") | .id'
# Per-provider params
curl "$NINEROUTER_URL/v1/models/info?id=firecrawl/fetch" -H "Authorization: Bearer $NINEROUTER_KEY"
```
