...
System config (including `LLM_API_KEY`) lives in `.env`. Skill-specific secrets (e.g. `BRAVE_SEARCH_API_KEY`) live in `.env.secrets` (git-ignored). Docker Compose loads both into the main service and only `.env.secrets` into the sidecar. Never put `LLM_API_KEY` in `.env.secrets`, because the sidecar runs arbitrary bash/python/js generated by the LLM. Copy from `.env.example` and `.env.secrets.example` respectively.
