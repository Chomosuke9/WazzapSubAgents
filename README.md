...
Both `.env` and `.env.secrets` are git-ignored. Docker Compose loads both files via `env_file` into the main service (`executor-service`), which runs the LLM agent loop. The sidecar (`executor-executor`) only receives skill-specific keys like `BRAVE_SEARCH_API_KEY` as explicit env vars — `LLM_API_KEY` is never injected there because the sidecar runs arbitrary bash/python/js commands generated by the LLM, which could leak env vars.
...
Both **`.env`** and **`.env.secrets`** are git-ignored. Docker Compose loads both via `env_file` into the main service only. The sidecar receives only skill-specific keys (`BRAVE_SEARCH_API_KEY`) as explicit env vars — `LLM_API_KEY` is never injected into the sidecar because it runs arbitrary commands generated by the LLM. `LLM_API_KEY` lives in **`.env`** alongside other config; skill-specific secrets like `BRAVE_SEARCH_API_KEY` live in **`.env.secrets`**. Copy from `.env.example` and `.env.secrets.example` respectively.
