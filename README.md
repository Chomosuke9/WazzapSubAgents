...
cp .env.secrets.example .env.secrets  # Secrets (BRAVE_SEARCH_API_KEY, etc.)
# Edit .env and .env.secrets with your values
```

Both `.env` and `.env.secrets` are git-ignored. Docker Compose loads both files into the main service (`executor-service`) and only `.env.secrets` into the sidecar (`executor-executor`). Never put `LLM_API_KEY` in `.env.secrets`: the sidecar runs arbitrary bash/python/js commands generated by the LLM. This lets skill-specific keys such as `BRAVE_SEARCH_API_KEY` reach the executor without relying on Compose interpolation.
...
Both **`.env`** and **`.env.secrets`** are git-ignored. Docker Compose loads both into the main service and only `.env.secrets` into the sidecar. Keep `LLM_API_KEY` exclusively in **`.env`** because the sidecar runs arbitrary commands generated by the LLM; put skill-only values such as `BRAVE_SEARCH_API_KEY` in **`.env.secrets`**. Copy from `.env.example` and `.env.secrets.example` respectively.
